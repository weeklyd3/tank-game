...
function updateGame() {
	...
	draw.textSize(50);
	draw.fill('black');
	if (!paused) draw.text(`${moneyCollected} coins earned, ${kills} kill(s), ${hits} hits`, 0, 20, width);
	draw.push();
	var now = Date.now();
    ...
    draw.textAlign(draw.CENTER, draw.CENTER);
	draw.textSize(25);
    if (equippedGuns[guns[selected-1]][3] > 1000000000){
    	...
    	draw.text(`Ammo: ${equippedGuns[guns[selected-1]][3]}`, 750, 870, 500);
    }
    draw.pop()
	...
	if (bubbleFrames < maxBubbleFrames) draw.line(width / 2, height / 2, width / 2 + movePointTowardsAnotherPoint(width / 2, height / 2, mouseX, mouseY, 70)[0], height / 2 + movePointTowardsAnotherPoint(width / 2, height / 2, mouseX, mouseY, 70)[1]);
	setOpacity(draw, 1);
    ...
    Object.keys(levelInfo.enemies).forEach(function(key) {
        ...
        var newEnemies = [];
        for (var i = 0; i < value.length; i++){
			if (!enemies[key].noCannon && Date.now() - value[i].frames >= enemies[key].cooldown && dist(x, y, value[i].x + value[i].sizeX/2, value[i].y + value[i].sizeY/2) <= 1250 && value[i].health > 0) {
			...
			if (value[i].motile) {
                ...
                draw.image(images.enemies[key].body, value[i].x-x+width/2, value[i].y-y+height/2, value[i].sizeX, value[i].sizeY);
                draw.pop();
			}
            else draw.image(images.enemies[key].body, value[i].x-x+width/2, value[i].y-y+height/2, value[i].sizeX, value[i].sizeY);
			draw.push();
			...
			if (value[i].health <= 0) {
                if (!value[i].deadFrames) coins.push([randint(value[i].x+25, value[i].x + value[i].sizeX-25), randint(value[i].y+25, value[i].y + value[i].sizeY-25), randint(enemies[key].money[0], enemies[key].money[1])]);
                if (!value[i].deadFrames){kills++; if (Math.random() < 0.35) medpacks.push([randint(value[i].x+25, value[i].x + value[i].sizeX-25), randint(value[i].y+25, value[i].y + value[i].sizeY-25)]);}
				value[i].health = 0;
    ...
    for (var i = 0; i < coins.length; i++){
        ...
        if ((coins[i][0] - x)*(coins[i][0] - x) + (coins[i][1] - y)*(coins[i][1] - y) < 3600){
    ...
    for (var i = 0; i < medpacks.length; i++){
        ...
        if (colliding({"x":x, "y":y, "r":35}, {"x":medpacks[i][0], "y":medpacks[i][1], "w":50, "h":40})){
            hp += 25;
            hp = Math.min(oghp, hp);
        }
	...
	draw.line(width / 2 - x, height / 2 - y, width / 2 - x, 3 * height / 2 - y);
	draw.line(width / 2 - x, 3 * height / 2 - y, levelInfo.length - x + width / 2, 3 * height / 2 - y);
	draw.line(levelInfo.length - x + width / 2, height / 2 - y, levelInfo.length - x + width / 2, 3 * height / 2 - y);
	draw.fill('black');
	for (var i = 0; i < levelInfo.length; i += 100) {
		draw.text(i, i - x + width / 2, -y+height/2);
		draw.text(i, i - x + width / 2, -y+height/2 + height);
	}
	...
	if (hp > 0) {
		if (!mousemode) {
			...
			if (Math.sqrt((centerx - mousex) ** 2 + (centery - mousey) ** 2) < 35 || draw.keyIsDown(66)) {
				xmul = 0;
				ymul = 0;
			}
			x += spd * xmul;
	...
	mouseX = draw.mouseX;
	mouseY = draw.mouseY;
	if ((firing || (mousemode && draw.keyIsDown(32))) && Date.now() - cooldowns[selected] >= weapons[guns[selected - 1]]["cooldown"] && hp > 0) {
        if (equippedGuns[guns[selected-1]][3] > 0){
    ...
    for (var i = 0; i < missiles.length; i++){
        if ((x-missiles[i].x)*(x-missiles[i].x) + (y-missiles[i].y)*(y-missiles[i].y) <= (35 + missiles[i].size)*(35 + missiles[i].size)){
			hits++;
		...
		Object.keys(levelInfo.enemies).forEach((k) => {
			...
			array.forEach((enemy, index) => {
				...
				if (enemy.health > 0 && colliding({
					...
					collided = bullets[i].dmg <= levelInfo.enemies[k][index].health;
					if (!collided) bullets[i].dmg -= levelInfo.enemies[k][index].health;
